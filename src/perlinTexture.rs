use crate::perlinNoise::perlin;
use crate::settings;
use macroquad::color::Color;
use macroquad::prelude::{Image, Texture2D, rand, screen_width};
...
fn perm() -> [i32; 512] {
    ...
    for idx in 0..512 {
        p[idx] = permuation[idx % 256];
    }

    return p;
}
pub fn perlin_texture(time: f32) -> Texture2D {
    let w = screen_width() as i32 ;
    let h = screen_height() as i32;
    let p = perm();
    let bg = settings::ui::BACKGROUND_COLOR;
    let scale = w as f32 / 32f32;

    let pixel_size = 20;
    let small_w = w / pixel_size;
    let small_h = h / pixel_size;

    let mut image = Image::gen_image_color(w as u16, h as u16, bg);

    for x in 0..small_w {
        for y in 0..small_h {
            let brightness = perlin(
                x as f32 / scale + time * 0.03,
                y as f32 / scale + time * 0.05f32,
                &p,
            );
            ...
            for px in 0..pixel_size {
                for py in 0..pixel_size {
                    let final_x = x * pixel_size + px;
                    let final_y = y * pixel_size + py;
                    if final_x < w && final_y < h {
                        image.set_pixel(final_x as u32, final_y as u32, color);
                    }
                }
            }
        }
    }
    return Texture2D::from_image(&image);
}
